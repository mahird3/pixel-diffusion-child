...
class ConvNextBlock(nn.Module):
    """Smaller ConvNeXt block with lighter convolutional layers"""

    def __init__(self, dim, dim_out, *, time_emb_dim=None, mult=2, norm=True):  # ⬅️ mult reduced from 4 → 2
        super().__init__()
        ...
        self.ds_conv = nn.Conv2d(dim, dim, kernel_size=5, padding=2, groups=dim)  # ⬅️ 7 → 5 kernel

        self.net = nn.Sequential(
            LayerNorm(dim) if norm else nn.Identity(),
            nn.Conv2d(dim, dim_out * mult, 3, padding=1),
            nn.GELU(),
            nn.Conv2d(dim_out * mult, dim_out, 3, padding=1)  # ⬅️ remove middle GELU + layer
        )
...
class LinearAttention(nn.Module):
    """Lighter attention block for lower memory usage"""

    def __init__(self, dim, heads=4, dim_head=32):  # ⬅️ smaller attention config
        super().__init__()
